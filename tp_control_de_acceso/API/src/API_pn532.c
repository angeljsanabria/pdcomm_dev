 ...
 *
 * @param 	addr_module - Direccion del esclavo (formato HAL I2C; en SPI sin
 * uso o reservado)
...
// };
static const uint8_t PN532_SAM_NORMAL_FRAME[] = {
    PN532_PROTOCOL_PREAMBLE_FRAME_1,
    PN532_PROTOCOL_PREAMBLE_FRAME_2,
    PN532_PROTOCOL_PREAMBLE_FRAME_3,
    0x05,
    0xFB,
...
PN532_Status_t PN532_init_module(PN532_CONFIG_MODE_T modo,
                                 PN532_Device_t dispositivo) {
  if (dispositivo != PN532_DEV_I2C) {
    /* SPI: tramas y chip select distintos; no soportado aun. */
    return PN532_ERR_OTRO;
  }
  pn532.dispositivo = dispositivo;
  pn532.addr_module = PN532_I2C_ADDR_SHIFT;
...
PN532_Status_t PN532_read_ack(void) {
  ...
  uint8_t i;
  uint8_t found;
  uint8_t falta_preambulo;

  n = 0;
  found = 0;
  falta_preambulo = 1;

  /* Si solo hubo 0x01 byte a byte,aun esta processando. */
  if (found == 0) {
    if (pn532_port_read(pn532.dispositivo, pn532.addr_module, buff_rx, 7,
                        pn532.op_timeout_ms) != PN532_PORT_OK) {
  ...
  if ((uint8_t)(n + 3) >
      PN532_READ_ACK_BUFFER_LEN) { // No tengo espacio para la respuesta ack
    return PN532_ERR_PROTOCOL;
  }

  if (falta_preambulo != 0) {
    if (pn532_port_read(pn532.dispositivo, pn532.addr_module, &buff_rx[n], 3,
                        pn532.op_timeout_ms) != PN532_PORT_OK) {
      return PN532_ERR_I2C;
    }
  }
...
static PN532_Status_t PN532_config_module_normal(void) {
  if (pn532_port_write(pn532.dispositivo, pn532.addr_module,
                       (const uint8_t *)PN532_SAM_NORMAL_FRAME,
...
PN532_Status_t PN532_read_inlist_response(void) {
  ...
  if (pn532_port_read(pn532.dispositivo, pn532.addr_module, buff_rx,
                      PN532_READ_ACK_BUFFER_LEN,
                      pn532.op_timeout_ms) != PN532_PORT_OK) {
    return PN532_ERR_I2C;
  }
   ...
   * Si antes viene solo ACK (00 00 FF 00 FF 00), LEN+LCS no suma 0x100: seguir
   * buscando. Ejemplo valido: 01 | 00 00 FF 0C F4 D5 4B 01 01 ... (0x01 listo
   * I2C opcional).
   */
  found = 0;
  p = 0;
  ...
  /* LEN = TFI + PD0..PDn (sin DCS). DCS va despues; checksum =
   * sum(TFI..PDn+DCS) mod 256 == 0. */
  ...
  /* Tras D5 4B: NbTg, Tg, ATQA(2), SAK, longitud UID, UID (NXP / mismo mapa que
   * Adafruit). */
...
PN532_Status_t PN532_polling_send(void) {
  if (pn532_port_write(pn532.dispositivo, pn532.addr_module,
                       (const uint8_t *)PN532_POLLING_READ,
...
// // Deprecado, porque copiaba en bytes.
//{
...
PN532_Status_t PN532_save_read_uid_hex(uint8_t *out, uint8_t out_cap) {
  ...
  case 4:
    w = snprintf((char *)out, (size_t)out_cap, "%02X%02X%02X%02X",
                 (uint8_t)pn532.uid[0], (uint8_t)pn532.uid[1],
                 (uint8_t)pn532.uid[2], (uint8_t)pn532.uid[3]);
    break;
  default:
    return PN532_ERR_OTRO;
  }
